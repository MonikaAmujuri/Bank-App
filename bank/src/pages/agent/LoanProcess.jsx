...
const LoanProcess = () => {
  ...
  if (!loanDetails.amount || Number(loanDetails.amount) <= 0) {
    errors.loanDetails = "Please complete Loan Details before continuing.";
  }

  if (!loanDetails.loanType || !String(loanDetails.loanType).trim()) {
    errors.loanDetails = "Please complete Loan Details before continuing.";
  }

  if (!loanDetails.interestRate || Number(loanDetails.interestRate) <= 0) {
    errors.loanDetails = "Please complete Loan Details before continuing.";
  }
...
const validateEmploymentDetails = () => {
  ...
  if (!employmentDetails.companyName || !String(employmentDetails.companyName).trim()) {
    errors.employmentDetails = "Please complete Employment Details before continuing.";
  }

  if (!employmentDetails.location || !String(employmentDetails.location).trim()) {
    errors.employmentDetails = "Please complete Employment Details before continuing.";
  }

  if (!employmentDetails.salary || Number(employmentDetails.salary) <= 0) {
    errors.employmentDetails = "Please complete Employment Details before continuing.";
  }

  if (
    !employmentDetails.netHandSalary &&
    !employmentDetails.netsalary
  ) {
    errors.employmentDetails = "Please complete Employment Details before continuing.";
  } else if (
    Number(employmentDetails.netHandSalary || employmentDetails.netsalary) <= 0
  ) {
    errors.employmentDetails = "Please complete Employment Details before continuing.";
  }
...
const validateKYCDetails = () => {
  const errors = {};
  const kycDetails = loan?.kycDetails || {};

  if (!kycDetails.panNumber || !String(kycDetails.panNumber).trim()) {
    errors.kycDetails = "Please complete KYC Details before continuing.";
  }

  if (!kycDetails.aadharNumber || !String(kycDetails.aadharNumber).trim()) {
    errors.kycDetails = "Please complete KYC Details before continuing.";
  }

  if (!kycDetails.address || !String(kycDetails.address).trim()) {
    errors.kycDetails = "Please complete KYC Details before continuing.";
  }
...
const validateEducationDetails = () => {
  const errors = {};
  const educationDetails = loan?.educationDetails || {};

  if (
    !educationDetails.institutionName ||
    !String(educationDetails.institutionName).trim()
  ) {
    errors.educationDetails = "Please complete Education Details before continuing.";
  }

  if (
    !educationDetails.courseName ||
    !String(educationDetails.courseName).trim()
  ) {
    errors.educationDetails = "Please complete Education Details before continuing.";
  }
  ...
  if (
    ...
    !String(educationDetails.courseDuration).trim()
  ) {
    errors.educationDetails = "Please complete Education Details before continuing.";
  }

  if (
    !educationDetails.instituteLocation ||
    !String(educationDetails.instituteLocation).trim()
  ) {
    errors.educationDetails = "Please complete Education Details before continuing.";
  }

  if (
    !educationDetails.courseFee ||
    Number(educationDetails.courseFee) <= 0
  ) {
    errors.educationDetails = "Please complete Education Details before continuing.";
  }

  if (
    !educationDetails.studyYear ||
    !String(educationDetails.studyYear).trim()
  ) {
    errors.educationDetails = "Please complete Education Details before continuing.";
  }
  ...
  if (currentStep === 0) {
    errors = validateLoanDetails();
  } else if (currentStep === 1) {
    errors = isEducationLoan
      ? validateEducationDetails()
      : validateEmploymentDetails();
  } else if (currentStep === 2) {
    errors = validateKYCDetails();
  }
...
const steps = [
  "Loan Details",
  isEducationLoan ? "Education Details" : "Employment Details",
  "KYC Details",
  "Review & Submit",
];
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      {isReviewOnly ? (
        ...
        <>
          ...
          <section className="rounded-3xl bg-white p-8 shadow-sm">
            {currentStep === 0 && stepErrors.loanDetails && (
  <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-600">
    {stepErrors.loanDetails}
  </div>
)}

{currentStep === 1 && isEducationLoan && stepErrors.educationDetails && (
  <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-600">
    {stepErrors.educationDetails}
  </div>
)}

{currentStep === 1 && !isEducationLoan && stepErrors.employmentDetails && (
  <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-600">
    {stepErrors.employmentDetails}
  </div>
)}

{currentStep === 2 && stepErrors.kycDetails && (
  <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-600">
    {stepErrors.kycDetails}
  </div>
)}
            {currentStep === 0 && (
              <LoanDetails
